--Prototyrant Concierge Prim (T-Helper 1 Cell)
...
function s.initial_effect(c)
	...
	Xyz.AddProcedure(c,s.MainFilter,6,3,s.CytokineFilter,aux.Stringid(id,5),3,s.XYZOperation)
	c:EnableReviveLimit()
	...
	e10:SetCondition(s.LifeSubtractCondition)
	e10:SetOperation(s.LifeSubtractOperation)
	...
	e13:SetOperation(s.EvasionOperation)
	c:RegisterEffect(e13)
...
function s.CytokineFilter(c,tp,lc)
	...
	and c:GetOverlayGroup():IsExists(s.IFNyIL12filter,1,nil)
end
function s.XYZOperation(e,tp,chk)
	if chk==0 then return Duel.IsCanRemoveCounter(tp,1,0,0x1b,1,REASON_EFFECT) end
	Duel.RemoveCounter(tp,1,0,0x1b,1,REASON_EFFECT)
	return true
end
...
function s.AnticipateCondition(e,tp,eg,ep,ev,re,r,rp)
	...
	Duel.IsCanRemoveCounter(tp,1,0,0x1b,1,REASON_COST)
end
...
--Battle Rule #4: Loss of Life (e10)
function s.LifeSubtractCondition(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():GetBattleTarget()~=nil
end
function s.LifeSubtractOperation(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
